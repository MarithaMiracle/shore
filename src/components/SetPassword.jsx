...
const SetPassword = () => {
  ...
  // Optional: Similar to ResetPassword, you could verify the token on mount
  // to give early feedback if the link is invalid/expired.
  useEffect(() => {
    // For now, we'll rely on the backend's POST request to validate the token.
    if (!token) {
      setIsValidToken(false);
      setMessage('No token found in the URL. Please use the link from your email.');
    }
    // You could also add a GET request here to check token validity against your backend
    // For example: fetch(`http://localhost:5000/auth/verify-set-password-token/${token}`);
  }, [token]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/auth/set-password/${token}`, {
        method: 'POST', // Backend expects POST for set-password
      ...
      console.error('Set password request error:', error);
